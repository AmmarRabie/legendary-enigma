...
public class ServerUtility {
    ...
    private static String BASE_URL = "http://" + IP + ":5000";
...
//        String encoding = Base64.encodeToString(("test1:test1").getBytes(‌"UTF‌​-8"​));
    ...
    public static String[] getResponse(String url, String inputJson, @Nullable String token) {
        ...
        try {
            ...
            status_line = httpResponse.getStatusLine().getStatusCode();

            if (status_line >= 200 && status_line < 300) {
                inputStream = httpResponse.getEntity().getContent();
                result[0] = readFromStream(inputStream);
            } else if (status_line >= 400 && status_line < 500) {
                inputStream = httpResponse.getEntity().getContent();
                result[1] = readFromStream(inputStream);
                Log.e(LOG_TAG, "Error response code: " + status_line);
            }
    ...
    private static String[] makeHttpRequest(URL url, String token, String authorization) throws IOException {
        ...
        try {
            ...
            if (status >= 200 && status < 300) {
                ...
                jsonResponse[0] = readFromStream(inputStream);
            } else if (status >= 400 && status < 500) {
                inputStream = urlConnection.getErrorStream();
                jsonResponse[1] = readFromStream(inputStream);
